# Load Balancer Shell Standard

#### Version 1.0.0


## Introduction
The Load Balancer Shell Standard is a project used to define a standard for all Load Balancer Shells that integrate with CloudShell. The standard defines the Shell’s data model, commands and a set of guidelines that should be followed in Load Balancer Shells development and customization.
...
Version | Date | Notes
--- | --- | ---
1.0.0 | 2018-04-09 | First release of the Load Balancer Shell Standard


## Definitions
### Granularity
A Load Balancer Shell should support all Load Balancer devices with the same Vendor and OS. 
...
### Generic Data Model
All Load Balancer Shells share the same generic data model, except the model of the root level which is different per each Shell. The data model shouldn’t be modified.
The attributes associated with those generic models are also shared by all Load Balancer Shells and their values are populated by the driver. It is allowed to add custom attributes only to the root level model, and it isn’t allowed to remove attributes from any level.

### Versioning
The Load Balancer Shell version follows Semantic Versioning Guidelines (see details in http://semver.org). In short, the version structure is Major.Minor.Patch, for example “1.0.2”. A Path version is promoted when making backward-compatibility bug fixes, a Minor version is promoted when adding functionality in a backwards-compatible manner and a  Major version is promoted when making a backwards incompatible changes.



## Data Model
### Families & Models
The Load Balancer shell standard supports Load Balancer devices.
...
- Family: Load Balancer, Model: Cisco IOS-SLB Load Balancer
 - Family: Server Farm, Model: Generic Server Farm
	- Family: Real Server, Model: Generic Real Server
...
Family | Rules
--- | ---
Load Balancer | Searchable
Server Farm | Searchable
Real Server | Searchable
Chassis | Searchable
Module | Searchable
...
--- | --- | --- | ---
Load Balancer | [Vendor] [OS] Load Balancer | (user defined) | (user defined - IP)
Server Farm | Generic Server Farm | Server Farm [ServerFarm Name] | SF [ServerFarm Name]
Real Server | Generic Real Server | RealServer[ID] | RS[ID]
...
Model Name | String | No | The device model name. This information will be used to display the model in the GUI.
...
#####  Generic Server Farms

Attribute Name | Data Type | User input? | Description
--- | --- | --- | ---
Name | String | No | The server farm name. 
Algorithm | String | No | The Algorithm used in this farm for balancing traffic
Virtual Server address | String | No | The Virual server address may be a url or an IP. 
Virtual server Port | String | No | The virtual server port number (either TCP or UDP).

#####  Generic Real Server
Attribute Name | Data Type | User input? | Description
--- | --- | --- | ---
Address | String | No | The real server address may be a url or an IP.
...
## Commands
Below is a list of all the commands that will be part of the standard Shell, their names and interfaces. Each Load Balancer Shell that will be released by Quali’s engineering will include implementation for all those commands.
...
###### Description
Performs checks on the device that validates that the Shell can work. In a Load Balancer device this checks usually include connectivity check for the protocols used by the Shell. The healtcheck result will be visible in the resource live status and command output.
...
Input / Output | Parameter | Alias | Data Type | Required | Description
--- | --- | --- | --- | --- | ---
Input | mode | mode | string | Yes | The save mode. Possible values are "Shallow" and "Deep". In a Load Balancer device both modes will have the same behavior of saving a configuration file.
...
The artifact types supported by Load Balancer orchestration_save command are "ftp", "tftp" and "filesystem".
...
{
  "saved_artifact_info": {
    "resource_name": "CiscoASALoad Balancer1",
    "resource_id" : "5F2EAA6C-E3FD-4DF0-8E2D-F05C81D61631",
    "created_date": "3577-04-27T00:17:48.819Z",
    "restore_rules": {
      "requires_same_resource": true
